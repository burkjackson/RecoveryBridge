...
17. **Stale `available` listeners no longer get pushed to — the query is fixed, the rows are still there** — `/api/notifications/send` used to target `role_state='available'` with no freshness check, so profiles hidden from every other list (they all filter on a 1-hour heartbeat) still received a push. Fixed 25 Aug: `lib/constants.ts` `isListenerOnline()` is now the single filter used by the send route, `AvailableListeners.tsx`, and `/listeners` — always_available bypasses the heartbeat (that's its whole point), a plain `role_state='available'` profile needs a fresh one. Measured 25 Aug: 10 profiles at `role_state='available'`, `always_available=false`, heartbeat stale — those stop receiving pushes; the always_available accounts are untouched by this, correctly (see #21, a separate problem). The rows themselves are unchanged — nobody was reset to `offline`, this only affects who gets notified. Whether the *previous* reach converted is instrumented (`notification_log`, migration 034); `supabase/queries/push_conversion.sql` still has historical data worth a look, but going forward the query behind it should stop counting these 10 as notified. There is also an unused `cleanup_stale_availability()` function in the database from migration 017 that resets the underlying rows if that's ever wanted.
