...
24. **Ending a session outside `/api/sessions/state` left the listener stuck offline (fixed 28 Aug 2026)** — the cleanup cron, `/api/account/delete`, and admin `block_user`/`end_session` all closed sessions with a raw `sessions` update and nothing else, so only the chat page's own end-of-chat flow (which calls `/api/sessions/state`) ever moved the listener back to `available`. Anyone whose session was closed by one of those other paths — the common case for an actually-abandoned chat, which is exactly what the cleanup cron exists to catch — stayed at `role_state='offline'` indefinitely: silently dropped from `AvailableListeners`, `/listeners`, and every support-request push until they happened to reopen the dashboard and re-toggle "I'm Here To Listen". Extracted the transition into `lib/serverSessionState.ts` (`endSessionRoleStates()`, seeker → offline, listener → available) and call it from all four server routes, plus the dashboard's `endAllActiveSessions` (client-side, on sign-out) now goes through the existing `syncSessionRoleStates` helper per session it ends, the same way the chat page always has. `block_user` skips restoring the blocked user to `available` when they were the listener — the whole point of blocking them is that they shouldn't go back in the pool. Same pass also fixed two smaller ones: `NotificationSettings.tsx` now clears its success-message timers on unmount (a save immediately followed by navigating away could call `setState` on a gone component), and the admin broadcast preview's `countPushable()` now requires an actual `push_subscriptions` row, not just the announcements toggle, matching what the drain route really delivers — it was overstating reach for anyone who opted in but never registered a device.
25. **8 listeners in the notification pool cannot be reached at all — outreach needed** — as of 24 Aug 2026 the pool (`role_state='available'` OR `always_available`) holds 17 profiles: 6 always-available (5 reachable), 1 available-and-fresh (reachable), and 10 available-but-stale, of which **only 2 are reachable**. The other 8 have no push subscription and no `email_notifications_enabled`, so they receive nothing — item #17's targeting fix does not even apply to them, and they never appear in `notification_log`. They are phantom capacity inflating the apparent listener count. Not re-measured since; the underlying counts may have moved with #17's 2-week auto-reset now running. This is not a data question and no further measurement will resolve it on its own — item #17 established that push is what makes a listener useful, which makes these 8 exactly the people worth converting. A drafted outreach message for this group exists ("Group A") and, as of 29 Aug, still has not been sent. Coverage remains the platform's binding constraint, so this is the highest-value open item.
26. **A direct-connect session now requires the listener to accept before the seeker can message (29 Aug 2026)** — reported from the listener's side: two overnight direct-connect requests were missed entirely, and both seekers had already been able to send a message, which read as an active conversation being ignored rather than a request nobody had seen yet. This deliberately reverses part of `f05e80c`/`284e082` (#10, 25 Aug), which chose transparency-plus-encouragement over blocking, reasoning from a 12s median reply time among *answered* sessions. Revisited against the fuller number from the same audit (`089148f`, #22): 96 of 202 sessions were "seeker wrote, listener never replied" — the reassuring copy optimized for the responsive half while the larger half kept writing into silence. `sessions.accepted_at` (migration 039, applied) is NULL for a pending direct-connect session and defaults to `now()` for every listener-initiated path, since choosing to connect already is acceptance. A restrictive RLS policy on `messages` INSERT is the actual enforcement — the listener's own messages are never blocked, so replying still counts as accepting; only the seeker side is gated. `app/chat/[id]/page.tsx` hides the composer and starter prompts on both sides while pending and shows the listener an Accept / Not now prompt instead. The "write your message straight away" copy in `AvailableListeners.tsx`'s confirm modal and the in-chat waiting note that invited an immediate message were rewritten to stop contradicting the gate; the last-active transparency line from `284e082` stays, since it's orthogonal and still useful. The 84-second-abandon case `284e082` was measuring is now handled by "Cancel request" instead of by encouraging a message into a room that might stay empty. See `supabase/migrations/README.md` (039) for the verification detail.

27. **The 039 accept gate was defeatable two ways — migrations 040 + 041, applied 31 Aug 2026** — `messages` carried three overlapping permissive INSERT policies, and permissive policies OR together, so the set was only as strong as its weakest member. `"Users can insert messages in their sessions"` asked only "are you in this session?" — no `sender_id = auth.uid()`, no `status` check. Either participant could therefore insert a message **attributed to the other person**, and into an already-ended conversation. It renders as that person's own words in the live chat, in `/history`, and in the admin transcript viewer that moderators read when judging a report. It also **defeated 039**: the accept gate permits an insert when `accepted_at is not null OR seeker_id is distinct from sender_id` (the branch that keeps a listener's reply unblocked), so a seeker in a pending session could satisfy it by forging the listener as the sender. Proven against production data in rolled-back transactions — seeker-as-self while pending was correctly blocked (42501), seeker-as-listener was ALLOWED both pending and ended. `040_message_sender_integrity.sql` drops the unsafe policy and the redundant twin of the safe one, leaving exactly one permissive INSERT rule; all six probes then match intent with every legitimate path still working. Note the Supabase security advisors do *not* flag this: it is policy logic, not a missing policy, so "advisors clean" says nothing about it. Whenever a table has more than one permissive policy for the same command, read them as an OR and check the weakest.

    **The second way in (041):** all three permissive UPDATE policies on `sessions` say only "are you the listener or the seeker?", and none carries a `WITH CHECK` — for an UPDATE, Postgres falls back to the `USING` expression, so the test applies to the *row* you touch, never the *columns* or the *values*. Acting as the seeker in rolled-back transactions: setting `accepted_at` on their own pending session was ALLOWED (that is the entire accept gate, switched off by the person it gates), swapping a third party in as `listener_id` was ALLOWED (`validate_session_participants()` is a BEFORE **INSERT** trigger, so an UPDATE walks past its availability/consent/block checks and drops an unconsenting user into a conversation), and flipping an `ended` session back to `active` was ALLOWED (a blocked user can reopen the room a moderator closed, since the messages policy only asks that the session be `active`). `041_protect_session_transitions.sql` is a BEFORE UPDATE trigger — a policy cannot see OLD, and these are statements about the *transition*. All three attacks now raise; all five legitimate writes still pass (listener Accept, seeker ends chat, seeker cancels pending, listener declines, service role ends session). **Both were applied 31 Aug 2026**, in order, and re-verified live against the real policies afterwards: five attacks blocked (forged listener message, seeker messaging while pending, self-accept, participant swap, reopening an ended session) and five legitimate paths working (listener replies while pending, listener Accept, seeker messages an accepted session, seeker ends chat, service role ends session), with no probe rows left behind and row counts unchanged. Either alone would have left the gate defeatable, which is why they went together.
28. **Realtime is not a delivery guarantee, and since 039 a dropped `sessions` UPDATE is user-visible (fixed 31 Aug 2026)** — chat has had a 3s message poll since the day a dropped INSERT was found to lose messages, but the session row itself had no fallback: `loadSession()` ran once on mount and the realtime UPDATE was the only refresh. That was survivable when the row only carried `status`; after 039 it carries `accepted_at`, which gates the seeker's composer. A dropped event left the seeker on "Waiting for X to accept..." with no way to reply while the listener was already typing at them. Added a session poll on the same 3s cadence, **scoped to the pending window only** (`status === 'active' && !accepted_at`), so an ordinary conversation pays nothing for it. Both delivery paths now go through one `applySessionUpdate()` helper so they cannot drift.
29. **A declined connection request asked the seeker to rate it (fixed 31 Aug 2026)** — `cancelPendingSession()` deliberately skips the feedback modal, "nothing happened yet for either side to rate" — but that only held for whoever pressed the button. The *other* party learned via the realtime UPDATE, which unconditionally set `feedbackModal`, so a seeker whose request was declined got "Was this conversation helpful?" over a 0-message summary, plus a thank-you-note box addressed to the person who had just turned them down. The modal is now gated on `accepted_at`, and the two "ended" surfaces say something true for a request that was never accepted rather than describing a conversation that never happened.
30. **A never-accepted request is not a past session (fixed 31 Aug 2026)** — `/history` selected every `status='ended'` session the user was in, so a direct connect the listener declined showed up as a past conversation with their name on it, for a seeker who had been gated out of even sending a message. Filtered on `accepted_at is not null`. The admin sessions tab deliberately still shows them: that view is the raw record, and a moderator wants to see the requests that died too.
31. **Still open, deliberately: a listener's reply does not itself accept.** 039 reasons that "if they're replying, they've self-evidently accepted" and its policy lets a listener's message through while pending on that basis — but nothing sets `accepted_at` when they do, so the seeker would stay gated. Unreachable through the app (the chat page hides the listener's composer until they press Accept) and left alone rather than papered over. If that branch ever needs to stand on its own, it wants an AFTER INSERT trigger on `messages` setting `accepted_at` on the listener's first message, not a widened policy.
...
```
npm test          # vitest — 98 tests across constants, favorites, timeWindows, rateLimit, errors, sessionState, missedConnections, notificationQueue
